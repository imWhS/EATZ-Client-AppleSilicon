//
//  SelectableKitchenwareManager.swift
...
protocol SelectableKitchenwareManager: ObservableObject {
    var selectedKitchenwares: [KitchenwareEssential] { get }
    func toggleSelection(for kitchenware: Kitchenware)
    func toggleSelection(for kitchenware: KitchenwareEssential)
    func isSelected(_ id: Int64) -> Bool
    func isDisabled(_ kitchenware: Kitchenware) -> Bool
    func complete()
}

enum SelectableKitchenwareAlert: Identifiable {
...
enum SelectableKitchenwareViewState {
    case loading
    case loaded
    case unauthorized
    case error(String)
