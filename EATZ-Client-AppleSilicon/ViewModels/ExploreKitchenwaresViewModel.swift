//
//  ExploreKitchenwaresViewModel.swift
...
class ExploreKitchenwaresViewModel: ObservableObject {
    ...
    @Published var alert: ExploreKitchenwaresAlert?
...
extension ExploreKitchenwaresViewModel {
    ...
    func handleItemAction(_ action: KitchenwareItemAction) {
        switch action {
    ...
    func addKitchenwareToPantry(_ id: Int64) {
    ...
    func removeKitchenwareFromPantry(_ id: Int64) {
    ...
    private func searchKitchenwares(keyword: String, page: Int = 0, completion: @escaping () -> Void = {}) {
    ...
    private func updatePantryStatus(for id: Int64, isOwned: Bool) {
        pagedSearchedKitchenwares.updateItem(for: id) { item in item.ownedByUser = isOwned }
        pagedKitchenwares.updateItem(for: id) { item in item.ownedByUser = isOwned }
    }
...
enum ExploreKitchenwaresViewState {
    case loading
    case loaded
    case unauthorized
...
enum ExploreKitchenwaresAlert: Identifiable {
    ...
    var alert: Alert {
        ...
        case .sessionExpired(let dismissAction):
            return Alert(
                title: Text("세션 만료"),
                message: Text("로그아웃 상태로 전환됐어요. 도구 둘러보기를 종료할게요."),
