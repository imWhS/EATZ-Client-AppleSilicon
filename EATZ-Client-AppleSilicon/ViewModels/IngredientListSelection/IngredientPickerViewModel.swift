...
/// 기존 재료 구성을 편집하기 위해, 재료 목록을 통해 1개 이상의 재료를
/// 선택 또는 해제할 수 있는 뷰 IngredientPicker에서 필요한 데이터와 로직을 제공합니다.
class IngredientPickerViewModel: ObservableObject, SelectableIngredientManager {
    ...
    /// 최종 선택된 재료 목록입니다.
    /// - 상위 뷰와 binding됩니다.
    private var selectionBinding: Binding<[IngredientEssential]>
    ...
    init(initialSelection: Binding<[IngredientEssential]>, auth: AuthProvider = AuthManager.shared) {
        self.selectionBinding = initialSelection
        self._selectedIngredients = Published(initialValue: initialSelection.wrappedValue)
        self.auth = auth
    }
    ...
    func complete() {
        selectionBinding.wrappedValue = selectedIngredients
        onDismiss?()
    }
        ...
        ingredientService.fetchRoots(page: page) { [weak self] result in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let response):
                    let ingredients = response.content.map { Ingredient(from: $0) }
