//
//  ExploreIngredientsViewModel.swift
...
class ExploreIngredientsViewModel: ObservableObject {
    ...
    @Published var alert: ExploreIngredientsAlert?
    
    // MARK: - 사용자 context 관련 프로퍼티
    
    @Published var searchKeyword = ""
    @Published var pagedSearchedIngredients: Paged<Ingredient> = .initial
    @Published var childIngredients: [Int64: [Ingredient]] = [:]
...
extension ExploreIngredientsViewModel {
    ...
    func loadChildIngredients(for parentId: Int64) {
        childListState[parentId] = .loading
        
        IngredientService.shared.fetchIngredientDetail(id: parentId) { [weak self] result in
            guard let self = self else { return }
            DispatchQueue.main.async {
                switch result {
                case .success(let response):
                    if let ingredients = response.children {
                        self.childIngredients[parentId] = ingredients.map { Ingredient(from: $0) }
                        self.childListState[parentId] = ingredients.isEmpty ? .empty : .loaded
                    } else {
                        self.childIngredients[parentId] = []
                        self.childListState[parentId] = .error(message: "정상적인 재료를 불러오지 못했어요.")
                    }
                case .failure(let networkError):
                    self.childListState[parentId] = .error(message: networkError.userMessage)
                }
            }
        }
    }
    ...
    private func loadIngredients(page: Int = 0, completion: @escaping () -> Void = {}) {
        IngredientService.shared.fetchRoots(page: page) { [weak self] result in
    ...
    private func searchIngredients(keyword: String, page: Int = 0, completion: @escaping () -> Void = {}) {
        IngredientService.shared.search(name: keyword, page: page) { [weak self] result in
            guard let self = self else { completion(); return }
            if case .unauthorized = self.viewState { completion(); return }
            ...
            DispatchQueue.main.async {
                ...
                case .success(let response):
                    ...
                    self.pagedSearchedIngredients.appendPage(
                        ingredients,
                        page: response.page,
                        hasNextPage: response.hasNext,
                        totalElements: response.totalElements)
    ...
    private func updateLikedStatus(for id: Int64, isLiked: Bool) {
        pagedIngredients.updateItem(for: id) { item in item.likedByUser = isLiked }
        pagedSearchedIngredients.updateItem(for: id) { item in item.likedByUser = isLiked }
        
        for (parentId, items) in childIngredients {
            if let index = items.firstIndex(where: {$0.id == id}) {
                childIngredients[parentId]?[index].likedByUser = isLiked
            }
        }
    }
    
    private func updatePantryStatus(for id: Int64, isOwned: Bool) {
        pagedIngredients.updateItem(for: id) { item in item.ownedByUser = isOwned }
        pagedSearchedIngredients.updateItem(for: id) { item in item.ownedByUser = isOwned }
        
        for (parentId, items) in childIngredients {
            if let index = items.firstIndex(where: {$0.id == id}) {
                childIngredients[parentId]?[index].ownedByUser = isOwned
            }
        }
    }
...
enum ExploreIngredientsViewState {
    case loading
    case loaded
    case empty
    case unauthorized
...
enum ExploreIngredientsAlert: Identifiable {
    ...
    var alert: Alert {
        ...
        case .sessionExpired(let dismissAction):
            return Alert(
                title: Text("세션 만료"),
                message: Text("로그아웃 상태로 전환됐어요. 재료 둘러보기를 종료할게요."),
