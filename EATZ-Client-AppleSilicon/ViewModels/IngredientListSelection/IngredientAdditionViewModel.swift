...
/// 재료 목록을 통해 1개 이상의 재료를 추가할 수 있는 뷰 IngredientAdditionView에서 필요한 데이터와 로직을 제공합니다.
class IngredientAdditionViewModel: ObservableObject, SelectableIngredientManager {
    
    // MARK: - 뷰 상태 프로퍼티 (View State Properties)
    
    @Published var searchState: SelectableIngredientSearchState = .searching
    
    /// 뷰가 보여줄 화면 상태
    /// - 뷰의 최상위 서브뷰에서 보여줄 화면을 분기하기 위해 사용합니다.
    @Published var viewState: SelectableIngredientViewState = .loading
    
    @Published var pagedIngredients: Paged<Ingredient> = .initial
    
    /// 화면에 표시할 alert
    /// - 아무 alert도 표시하지 않는 경우 `nil`이 됩니다.
    @Published var alert: SelectableIngredientAlert?
    
    // MARK: - 사용자 context 관련 프로퍼티
    
    @Published var searchKeyword = ""
    @Published var pagedSearchedIngredients: Paged<Ingredient> = .initial
    @Published var selectedIngredients: [IngredientEssential] = []
    ...
    func loadMoreIngredients() {
        guard pagedIngredients.hasNextPage, !pagedIngredients.isLoadingNextPage else { return }
    ...
    func loadMoreSearchedIngredients() {
        guard pagedSearchedIngredients.hasNextPage, !pagedSearchedIngredients.isLoadingNextPage else { return }
    ...
    func isSelected(_ id: Int64) -> Bool {
        selectedIngredients.contains { $0.id == id }
    }
    ...
    private func loadIngredients(page: Int = 0, completion: @escaping () -> Void = {}) {
        ingredientService.fetchRoots(page: page) { [weak self] result in
            guard let self = self else { completion(); return }
            
            DispatchQueue.main.async {
                switch result {
                case .success(let response):
                    let newIngredients = response.content.map { Ingredient(from: $0) }
                    
                    if page == 0 && newIngredients.isEmpty {
    ...
    private func addSelectionToPantry() {
        let ingredientIds = selectedIngredients.map { ingredient in return ingredient.id }
