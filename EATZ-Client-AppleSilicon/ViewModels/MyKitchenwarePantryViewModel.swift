//
//  MyKitchenwarePantryViewModel.swift
...
class MyKitchenwarePantryViewModel: ObservableObject {
    ...
    @Published var alert: MyKitchenwarePantryAlert?

    /// 화면에 표시할 sheet
    /// - 아무 sheet도 표시하지 않는 경우 `nil`이 됩니다.
    @Published var sheet: MyKitchenwarePantrySheet?
    ...
    func loadMoreKitchenwares() {
        ...
        loadKitchenwares(page: pagedKitchenwares.page + 1) { self.pagedKitchenwares.isLoadingNextPage = false }
    }
    
    func handleItemAction(action: KitchenwareItemAction) {
        switch action {
        case .addToPantry(let id): addKitchenwareToPantry(id)
        case .removeFromPantry(let id): removeKitchenwareFromPantry(id)
        }
    }
    ...
    private func loadKitchenwares(page: Int = 0, completion: @escaping () -> Void = {}) {
        UserPantryService.shared.fetchKitchenwares(page: page, size: 10) { [weak self] result in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let response):
                    ...
                    let kitchenwares = content.map { return Kitchenware(from: $0) }
                    self.pagedKitchenwares.appendPage(kitchenwares, page: response.page, hasNextPage: response.hasNext, totalElements: response.totalElements)
    ...
    private func clearPantry() {
        UserPantryService.shared.clearKitchenwares { [weak self] result in
    ...
    private func addKitchenwareToPantry(_ id: Int64) {
        updatePantryStatus(for: id, isOwned: true)
        UserPantryService.shared.addKitchenwares(ids: [id]) { [weak self] result in
            guard let self = self else { return }
    ...
    private func removeKitchenwareFromPantry(_ id: Int64) {
        updatePantryStatus(for: id, isOwned: false)
        UserPantryService.shared.removeKitchenwares(ids: [id]) { [weak self] result in
            guard let self = self else { return }
            if case .failure(let networkError) = result {
    ...
    private func updatePantryStatus(for id: Int64, isOwned: Bool) {
        pagedKitchenwares.updateItem(for: id) { item in
